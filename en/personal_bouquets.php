...
echo '<h5>__Bouquets</h5>';
echo '<div class="gap gap-2"></div>';

$dbh = db_connect();

$dlub_rs = $dbh->prepare("execute dbo.list_user_bouquets :usr");
$dlub_rs->bindParam(":usr", $user_id, PDO::PARAM_INT, 0);
$dlub_rs->execute();
if ($dbh->errorCode() == "00000") {
    echo '<table class="data_table">';
    echo '<tr><th>description</th><th>order</th><th colspan="3">action</th></tr>';
    $r = 0;
    $g = 0;
    while ($dlub_r = $dlub_rs->fetchObject()) {
        ...
        if ($g != $dlub_r->grp) {
            $g = $dlub_r->grp;
            echo '<tr><td colspan="5" class="block_description" style="background: #3fed98">' . $dlub_r->class_group . '</td></tr>';
        }
        echo '<tr>';
        ...
        echo '<td>';
        echo '<a href="personal_bouquet_edit.php?id=' . $dlub_r->id . '">edit</a>&nbsp;';
        echo '<a href="personal_bouquet_items.php?id=' . $dlub_r->id . '">items</a>&nbsp;';
        echo '<a href="personal_bouquet_delete.php?id=' . $dlub_r->id . '">delete</a>';
        echo '</td>';
        echo "</tr>";
    }
    echo '</table>';
    unset($dlub_rs);
    echo '<a href="personal_bouquet_edit.php">add&nbsp;new</a>';
}
