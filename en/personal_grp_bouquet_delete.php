...
echo '<h5>__Delete group bouquet?</h5>';
echo '<div class="gap gap-2"></div>';

$dbh = db_connect();

$dgbi_rs = $dbh->prepare("execute dbo.get_grp_bouquet_info :id, :usr");
$dgbi_rs->bindParam(":id", $id, PDO::PARAM_INT, 0);
$dgbi_rs->bindParam(":usr", $user_id, PDO::PARAM_INT, 0);
$dgbi_rs->execute();
if ($dbh->errorCode() == "00000") {
 while ($dgbi_r = $dgbi_rs->fetchObject()) {
  echo "<h5>".$dgbi_r->restaurant." / " . $dgbi_r->description."</h5>";

  echo "<a href=\"personal_grp_bouquet_do_delete.php?id=".$id."\">yes</a>&nbsp;&nbsp;&nbsp;<a href=\"personal_grp_bouquets.php\">no</a>";
  echo "<br>";
  echo "<br>";


  $dlubd_rs = $dbh->prepare("execute dbo.list_user_grp_bouquet_details :id, :usr");
